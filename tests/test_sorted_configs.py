...
def test_is_exported_file_copied():
    config_a = BuildConfig('image_a', dockerfile=Dockerfile('FROM busybox\nCOPY beep.txt /'))
    config_b = BuildConfig('image_b', dockerfile=Dockerfile('FROM busybox\nCMD ["/bin/true"]'))
...
def test_get_base_image():
    ...
    config = attr.assoc(config, dockerfile=Dockerfile('FROM busybox:latest'))
    assert get_base_image(config) == 'busybox'

    config = attr.assoc(config, dockerfile=Dockerfile('FROM busybox@00f017a8c2a6'))
    assert get_base_image(config) == 'busybox'
